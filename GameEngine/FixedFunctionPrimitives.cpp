...
void RenderNormals(const Frustum& frustum) {
	vec3 NTL = planeIntxn(frustum.near, frustum.top, frustum.left);
	vec3 NTR = planeIntxn(frustum.near, frustum.top, frustum.right);
	vec3 NBL = planeIntxn(frustum.near, frustum.bottom, frustum.left);
	vec3 NBR = planeIntxn(frustum.near, frustum.bottom, frustum.right);

	vec3 FTL = planeIntxn(frustum.far, frustum.top, frustum.left);
	vec3 FTR = planeIntxn(frustum.far, frustum.top, frustum.right);
	vec3 FBL = planeIntxn(frustum.far, frustum.bottom, frustum.left);
	vec3 FBR = planeIntxn(frustum.far, frustum.bottom, frustum.right);
...
void Render(const Circle& circle) {
	...
	for (int i = 0; i < 360; ++i) {
		glVertex2f(circle.center.x + cosf(DEG2RAD(i)) * circle.radius, circle.center.y + sinf(DEG2RAD(i)) * circle.radius);
	}
...
void Render(const OBB& obb) {
	...
	mat4 scal = scale(obb.size);
	mat4 rotation = FromMat3(obb.orientation);
	mat4 translat = translation(obb.center);

	// SRT: Scale First, Rotate Second, Translate Last
	// orientation = roll * pitch * yaw;
	mat4 transform = scal * rotation * translat;
...
void Render(const OrientedRectangle& rect) {
	glPushMatrix();
	glTranslatef(rect.center.x, rect.center.y, 0.0f);
	glRotatef(rect.rotation, 0.0f, 0.0f, 1.0f);

	vec2 min = vec2(-rect.extents.x, -rect.extents.y);
	vec2 max = vec2(+rect.extents.x, +rect.extents.y);
